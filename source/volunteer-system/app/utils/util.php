...
function isValidPhoneNumber($phoneNumber) {
    $splitphone = explode('-', $phoneNumber);
	if( preg_match("/^[0-9]{3}$/", $splitphone[0]) === 1 &&
		preg_match("/^[0-9]{3}$/", $splitphone[1]) === 1 &&
		preg_match("/^[0-9]{4}$/", $splitphone[2]) === 1) {
		return true;
	}
	return false;
}
...
function getLocationDirectionsURL($location) {
	if($location == "Orem") {
		$directionsURL = getURLForParentSite("location/kids-on-the-move-orem");
	}
	else if($location == "Salem") {
		$directionsURL = getURLForParentSite("?page_id=-1"); // no longer used; no directions page exists
	}
	else if($location == "Springville") {
		$directionsURL = getURLForParentSite("location/turn-community-services-springville");
	}
	else if($location == "Lehi") {
		$directionsURL = getURLForParentSite("location/kids-on-the-move-lehi");
	}
	else if($location == "St. George") {
		$directionsURL = getURLForParentSite("location/kids-on-the-move-st.-george");
	}
	else {
