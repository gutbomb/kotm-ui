...
$dbserver = "mysql.kotm.org";
if ($_SERVER["SERVER_NAME"] === "localhost") {
    $dbserver = "mysql";
}
$DB_CONN = mysqli_connect($dbserver,"kotm_vol_sys","", "kotm_vol_sys");
...
function getAccountByEmail($email, $filterAdmin=false) {
	global $DBT_PREFIX, $DB_CONN;
	$filterAdminQueryPart = "";
	if($filterAdmin) {
		$filterAdminQueryPart = " admin='1' AND";
	}
	$sql = sprintf("SELECT *, CONVERT_TZ(createDate,'UTC','America/Denver') createDate, CONVERT_TZ(lastLoginDate,'UTC','America/Denver') lastLoginDate FROM ".$DBT_PREFIX."account WHERE".$filterAdminQueryPart." email = '%s'",
...
function emailExists($email) {
	global $DBT_PREFIX, $DB_CONN;
	$sql = sprintf("SELECT * FROM ".$DBT_PREFIX."account WHERE email = '%s'",
					mysqli_real_escape_string($DB_CONN, $email));
	$result = mysqli_fetch_assoc( mysqli_query($DB_CONN, $sql) );
	if($result) {
		return true;
