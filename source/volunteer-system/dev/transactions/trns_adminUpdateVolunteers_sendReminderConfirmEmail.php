...
if(!isErrors()) {
	if($vDate['type'] == "Respite") {
		require_once('emails/respite/email_signupConfirm.php');
	}
	else {
		require_once('emails/childcare/email_signupConfirm.php');
	}
	for($i=0; $i<count($accts); $i++) {
		$confirmId = mysqli_fetch_assoc( getVolunteerConfirmId($accts[$i]['uname'], $vDate['id']) );
		if($confirmId && $confirmId['confirmId'] != null) {
			$confirmId = $confirmId['confirmId'];
		}
		else {
			$confirmId = generateRandomPassword(20);
			setVolunteerConfirmId($accts[$i]['uname'], $vDate['id'], $confirmId);
		}
		if($STAGE == "DEV") {
		...
		$link .= "?page=confirmSignup&uname=".$accts[$i]['uname']."&vDateId=".$vDate['id']."&confirmId=$confirmId";
		sendSignupConfirmEmail($accts[$i], $vDate, $link);
	}
